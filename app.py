...
app = Flask(__name__)
app.secret_key = 'supersecretkey'  # Chave secreta para o gerenciamento da sessão
...
def home():
    # Inicializa ou reseta o histórico de chat na sessão
    if 'chat_history' not in session or request.method == 'GET':
        session['chat_history'] = ["🤖 Emabot: Olá, eu sou a Emabot da Diplan. Sou seu assistente de busca... Como posso ajudar?"]
    ...
    if request.method == 'POST':
        ...
        session['chat_history'].append(user_message)  # Adiciona a interação atual ao histórico
        
        results = search_in_spreadsheet(user_input)
        if results:
            session['chat_history'].append("🤖 Emabot: Documentos encontrados:")
            for result in results:
                session['chat_history'].append(f"📄 <a href='/get_link?title={result['Título do documento']}'> {result['Título do documento']}</a>")
        else:
            session['chat_history'].append("🤖 Emabot: Nenhum documento encontrado com essas palavras-chave.")
...
def get_link():
    ...
    if not result.empty:
        ...
        session['chat_history'].append(f"🤖 Emabot: Aqui está o link para '{title}': <a href='{link}' target='_blank'>{link}</a>")
    else:
        session['chat_history'].append("🤖 Emabot: Link não encontrado para o título selecionado.")
